...
public class BaseHandInfo {
    ...
    static Obb arm2 = new Obb("arm2",
            ...
            new Vector[]{new Vector(0,0,0)
                    ,new Vector(0,0,0)
                    ,new Vector(0,0,0)},
            new double[]{arm2_x,arm2_y,arm2_z});
    static Obb dipper = new Obb("dipper",
            new Point(0,0,0),
            new Vector[]{new Vector(0,0,0)
        ,new Vector(0,0,0)
            ,new Vector(0,0,0)},
            new double[]{dipper_x,dipper_y,dipper_z});
    static Obb bucket = new Obb("bucket",
            new Point(0,0,0),
            new Vector[]{new Vector(0,0,0)
                    ,new Vector(0,0,0)
                    ,new Vector(0,0,0)},
            new double[]{bucket_x,bucket_y,bucket_z});
    ...
    public static  void  changehand(handJointInfo  currentHandJointInfo) {
        ...
        double theta2t=currentHandJointInfo.theta2+diffTheta2Theta2t;
        ...
        arm1.vectors[1].vextorX =   Utils.sin(theta1);
        arm1.vectors[1].vextorY =   -Utils.sin(theta1);
        arm1.vectors[1].vextorZ =   0;
        ...
        pc.x=L1_AB*Utils.cos(theta1)*Utils.cos(theta2)+a0*Utils.cos(theta1);
        pc.y=L1_AB*Utils.cos(theta2)*Utils.sin(theta1)+a0*Utils.sin(theta1);
        pc.z=L1_AB*Utils.sin(theta2)+d0;
        ...
        pd.x=pc.x+L2*Utils.cos(theta2)*Utils.cos(theta2+theta3);
        pd.y=pc.y+L2*Utils.cos(theta2+theta3)*Utils.sin(theta1);
        pd.z=L2*Utils.cos(theta2+theta3);
        ...
        bucket.vectors[1].vextorY=Utils.cos(thetaPoint);
